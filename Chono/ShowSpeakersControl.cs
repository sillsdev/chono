	...
	public partial class ShowSpeakersControl : EmbeddedPluginControl
	{
		...
		private void HandleStringsLocalized(ILocalizationManager lm = null)
		{
			if (lm != null && lm != ChonoPlugin.PrimaryLocalizationManager)
				return;
			SetWindowTitle();
			if (LocalizationManager.UILanguageId == ChonoPlugin.kDefaultUILocale)
			{
		...
		private void SetWindowTitle()
		{
			...
			m_parentWindow.SetTitle(reference == null ?
				LocalizationManager.GetString("Chono.ShowSpeakers.WindowTitleNoReference",
					"Speaking characters",
					"Project name will be appended to this") :
				string.Format(LocalizationManager.GetString("Chono.ShowSpeakers.WindowTitle",
